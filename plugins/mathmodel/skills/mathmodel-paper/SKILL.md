...
description: "数学建模竞赛论文撰写工具，支持 Typst 和 LaTeX 双引擎。根据建模/结果报告和 figures/*.pdf 选择比赛模板、排版引擎、组织章节，在正文中按章节直接插入图表与文献引用。可独立触发，无需前序流程。"
---

# 竞赛论文撰写（Typst / LaTeX）

本工具负责把已有的建模分析、结果数据和图表组织成竞赛论文：选择比赛模板和排版引擎、组织论文结构、决定每张图表放入哪个章节、把文献调研产出落成论文引用。

- 输入（存在则使用）：`reports/ANALYSIS_MODELING_REPORT.md`、`reports/RESULTS_REPORT.md`、`reports/LITERATURE_REVIEW.md`、`figures/*.pdf`、`literature/<题目>/references.bib`。
- 前序文件缺失时：以用户当前提供的材料为准，不强行要求补齐。
- **Typst 引擎**写法参考本文件模板与"Typst 模板章节"小节；**LaTeX 引擎**参考本文件末尾的"LaTeX 写作要点"小节。

## 环境约定（运行前必读）

1. 读工作目录的 `.mathmodel_env.md` 获取"排版引擎 / 竞赛类型 / 论文语言"偏好；已记录则沿用（向用户确认一句即可）。
2. 未记录时以对话方式询问用户，并把答案写入 `.mathmodel_env.md`。
3. 论文中的所有数值图表结论必须来自 `reports/RESULTS_REPORT.md` 或 `figures/*`，不得编造、估算或使用不同的四舍五入方式。
...
**撰写论文前必须确定排版引擎。** 引擎决定后续所有步骤（模板路径、章节文件扩展名、图片插入语法、编译命令），选错会导致整篇论文格式错误。

先读 `.mathmodel_env.md` 的"排版引擎"字段：

- 已记录：向用户确认一句"沿用 <LaTeX/Typst> 吗？"，沿用或改选。
- 未记录：以对话方式询问用户"撰写论文使用哪种排版引擎？"
  - 选项 1：LaTeX（xelatex 编译，数学建模竞赛主流，模板已全部就绪）— 推荐选项放第一位
  - 选项 2：Typst（typst 编译）
- 用户未明确指定时，**默认使用 LaTeX**，并把选择写入 `.mathmodel_env.md`。

根据确定的引擎选择对应模板族：

- **Typst 引擎**：使用 `templates/<lang>/<竞赛>/main.typ`。编译命令 `typst compile main.typ`。
...
**只使用真实存在的参考文献**（每个条目都有可验证的 arXiv ID/DOI/URL）。禁止编造文献。

**文献来源优先级**：

1. `literature/<题目>/references.bib`（由 `$mathmodel-lit-review` 产出，已验证真实性）——有则直接使用。
2. `reports/LITERATURE_REVIEW.md` 中的文献清单。
3. 两者都没有时，只使用用户提供或模型在写作过程中**实际打开验证过**的文献。

文件名按引擎选择：Typst 用 `paper/references.typ`，LaTeX 用 `paper/references.tex`。

**从 references.bib 落引用**：BibTeX 条目转写为模板使用的编号引用格式（转写时保留作者、标题、期刊/会议、年份、卷期页码，并保留 arXiv/DOI 标识供读者检索）。
...
**引用位置**：在正文介绍模型依据、相关工作或借鉴方法的段落插入上标引用；参考文献编号顺序与正文首次出现顺序一致。`$mathmodel-verify` 会检查正文引用与文献条目的一致性。
...
以下要点供 **LaTeX 引擎**使用。Typst 引擎的语法以本文件"模板族"和模板章节文件中的实际写法为准。
