---
name: mathmodel-modeling
description: "数学建模赛题分析与建模设计工具。用于读取题面和附件，完成子问题拆解、数据理解、假设预检、变量定义、模型公式、目标函数、约束条件、求解策略和可交给代码实现的建模报告。可独立触发，也可参考文献调研报告借鉴方法。"
---
...
如需领域判断，读取 `../mathmodel-references/math_modeling_norms.md` 中的“赛题理解与子问题识别”“假设与模型建立”和“题型防错速查”小节。该文件只作为规范知识库，不替代本工具的分析报告结构。
...
不要在本工具写论文正文，不要生成最终 `paper/`，不要把图表排版任务提前到这里。
